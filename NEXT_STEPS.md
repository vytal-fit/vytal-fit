...
- **`pro`**: **só 6 referências a `useDataStore` restantes** (audit 2026-06-30, fim de sessão), todas esperadas: `financials` (F5, diferida), `layout.tsx` (só `switchOrgData`, intencional) e `members/[id]/{assessments,body,nutrition,referrals}` (D6, diferidas). **Tudo o resto está em tRPC real**: shell (notificações+features+mensagens), dashboard, members (list/[id]/edit/cancel/billing/360/import), staff (list/[id]), classes (list/[id]/attendance/feedback/waitlist), WODs (+programming), CRM (kanban + [id]), plans (list/create), locations, class-types, exercises, check-ins, settings (geral/features/payments/website), integrations, dropins, screen, community, communications/sms, store, mensagens.
  ...
  - **Member billing + payments tab + financials/dunning**: ligados a `payments` real (`payments.byMember`, `payments.list({status:"overdue"})`). Pagamentos do membro, totais, e fila de dunning (tentativas/retry projetados dos dias em atraso) são reais.
  - **O que falta (só intencional + D6 + F5-fiscal)**: `layout.tsx` → só `switchOrgData` (intencional); `members/[id]/{body,nutrition,assessments,referrals}` → D6 (não fazer). **Sub-páginas F5-fiscais** (`financials/{invoices,expenses,revenue,budget,sepa}`) são ecrãs demo estáticos (não usam `useDataStore`) que precisam de modelos próprios da F5 — faturas com linhas/IVA/ATCUD, despesas, orçamento, mandatos SEPA, export SAF-T — a fazer no build F5 com PRD dedicado e fornecedor certificado. **Feeds sociais / campanhas SMS / eventos-badges** continuam ilustrativos por faltarem tabelas próprias.

> **Estado da estrela-guia (fim desta sessão):** `pro` e `my` sem `useDataStore` para dados de domínio (exceto `switchOrgData` intencional + 4 páginas D6). 19 migrações (0013→0019) em prod. Routers reais: leads(+activities), messages, bookings(+attendance/waitlist), orgSettings(+profile/payments/dropins/website/slug), payments. **622 testes.** Falta só F5-fiscal (PRD dedicado) e D6.
