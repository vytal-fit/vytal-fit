...
- **`my`**: 1 página em mock — `page` (home). Já reais: `/wellness`, `records`, `profile`, `wod`, `schedule` (aulas via `classes.schedule` enriquecido + reservas via `bookings.book/cancel/listByMember`).
