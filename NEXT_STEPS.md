...
- **Ainda subsistemas próprios**: `automations/campaigns` (sequências drip com triggers + multi-email + analytics de open/click → precisa de scheduler + tracking; o engine de broadcast + comms-policy é a base a reutilizar), `marketing` (agendador de redes sociais IG/FB/LinkedIn → integrações + scheduler), `media` (storage de assets → blob + tabela assets, `uploads` é a costura). **comunidade: sistema de reports/flagging + desafios/competições/eventos** (tabs Moderação e Desafios do `pro/community` ficam em empty-state honesto até lá) + badges/questionnaires/events (páginas próprias).
