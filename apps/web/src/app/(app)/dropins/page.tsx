...
import {
  Globe, MapPin, Save, DollarSign, ShieldCheck, Image,
} from "lucide-react";
...
export default function DropInsPage() {
  ...
  function handleSave() {
    // Persist drop-in settings alongside org settings
    updateOrgSettings({
      slogan: orgSettings.slogan, // keep existing
    });
    toast("Drop-in settings saved successfully", "success");
  }
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        ...
        <div className="space-y-6">
          ...
          <div className="rounded-xl border border-vytal-border bg-vytal-card p-6 space-y-5">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="text-sm font-semibold text-vytal-text">Drop-in Active</p>
                  <p className="text-xs text-vytal-muted">Allow visitors to register for drop-in sessions</p>
                </div>
              </div>
              <button
                type="button"
                onClick={() => setActive(!active)}
                className={cn("relative h-6 w-11 rounded-full transition-colors", active ? "bg-vytal-green" : "bg-vytal-bg3")}
              >
                <span className={cn("absolute top-0.5 h-5 w-5 rounded-full bg-white transition-transform", active ? "left-[22px]" : "left-0.5")} />
              </button>
            </div>

            <div>
              <label className="mb-1.5 block text-xs font-medium uppercase tracking-wider text-vytal-muted">Drop-in Price (EUR)</label>
          ...
          <div className="rounded-xl border border-vytal-border bg-vytal-card p-6 space-y-4">
            <div className="flex items-center gap-2">
              ...
              <h3 className="text-sm font-semibold text-vytal-text">Location</h3>
            </div>
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className="mb-1 block text-[10px] font-medium uppercase tracking-wider text-vytal-muted">Latitude</label>
                <input type="text" value={lat} onChange={(e) => setLat(e.target.value)} className="w-full rounded-lg border border-vytal-border bg-vytal-bg2 px-3 py-2 text-sm text-vytal-text focus:border-vytal-green/30 focus:outline-none focus:ring-1 focus:ring-vytal-green/20" />
              </div>
              <div>
                <label className="mb-1 block text-[10px] font-medium uppercase tracking-wider text-vytal-muted">Longitude</label>
                <input type="text" value={lng} onChange={(e) => setLng(e.target.value)} className="w-full rounded-lg border border-vytal-border bg-vytal-bg2 px-3 py-2 text-sm text-vytal-text focus:border-vytal-green/30 focus:outline-none focus:ring-1 focus:ring-vytal-green/20" />
              </div>
            </div>
            <div className="flex h-40 items-center justify-center rounded-lg border border-dashed border-vytal-border bg-vytal-bg2">
              <div className="text-center">
                ...
                <p className="mt-2 text-xs text-vytal-muted">Map preview ({lat}, {lng})</p>
              </div>
            </div>
          </div>
          ...
          <div className="rounded-xl border border-vytal-border bg-vytal-card p-6 space-y-4">
            ...
            <div className="grid grid-cols-3 gap-3">
              {[1, 2, 3, 4, 5, 6].map((n) => (
                <div key={n} className="flex aspect-square items-center justify-center rounded-lg border border-dashed border-vytal-border bg-vytal-bg2 transition-colors hover:border-vytal-green/30">
        ...
        <div className="space-y-6">
          ...
          <div className="rounded-xl border border-vytal-border bg-vytal-card p-6 space-y-5">
            ...
            <div>
              <label className="mb-1.5 block text-xs font-medium uppercase tracking-wider text-vytal-muted">Who can register</label>
              <div className="flex gap-3">
                {([{ value: "all", label: "Members + Visitors" }, { value: "members", label: "Members Only" }] as const).map((opt) => (
            ...
            <div className="flex items-center justify-between">
              ...
              <button type="button" onClick={() => setEmailValidation(!emailValidation)} className={cn("relative h-6 w-11 rounded-full transition-colors", emailValidation ? "bg-vytal-green" : "bg-vytal-bg3")}>
                <span className={cn("absolute top-0.5 h-5 w-5 rounded-full bg-white transition-transform", emailValidation ? "left-[22px]" : "left-0.5")} />
              </button>
            </div>
            <div>
              <label className="mb-1.5 block text-xs font-medium uppercase tracking-wider text-vytal-muted">Required Details</label>
              <div className="flex gap-3">
                {([{ value: "full", label: "Full (Name, Email, Phone, ID)" }, { value: "basic", label: "Basic (Name, Email)" }] as const).map((opt) => (
            ...
            <div>
              <label className="mb-1.5 block text-xs font-medium uppercase tracking-wider text-vytal-muted">Absence Limit (no-shows)</label>
          ...
          <div className="rounded-xl border border-vytal-border bg-vytal-card p-6 space-y-4">
            <div className="flex items-center justify-between">
              <h3 className="text-sm font-semibold text-vytal-text">Details</h3>
