...
describe("getSignInUrl", () => {
  ...
  it("stays same-origin in dev/preview when NEXT_PUBLIC_ADMIN_URL is unset", () => {
  ...
  it("points at the configured admin host in production", () => {
  ...
  it("does not double the slash when the admin URL has a trailing slash", () => {
    process.env.NEXT_PUBLIC_ADMIN_URL = "https://pro.vytal.fit/";
    expect(getSignInUrl()).toBe("https://pro.vytal.fit/login");
  });
});
