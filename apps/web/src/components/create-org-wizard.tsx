...
function Confetti() {
  const particles = useMemo(() => {
    const colors = ["#22c55e", "#00d4ff", "#ffb300", "#c084fc", "#ff8c42", "#ff4757"];
...
function AnimatedCheckmark() {
  return (
    <div className="relative mx-auto mb-6 flex h-24 w-24 items-center justify-center">
      ...
      />
      <svg className="h-24 w-24" viewBox="0 0 52 52" fill="none" xmlns="http://www.w3.org/2000/svg">
        <circle className="onboarding-checkmark-circle" cx="26" cy="26" r="25" fill="none" stroke="#22c55e" strokeWidth="2" />
        <path className="onboarding-checkmark-check" d="M14.1 27.2l7.1 7.2 16.7-16.8" fill="none" stroke="#22c55e" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round" />
      </svg>
    </div>
  );
}
...
export function CreateOrgWizard({ onComplete, onCancel, isModal }: CreateOrgWizardProps) {
  const { t } = useI18n();
  ...
  const featuresByCategory = useMemo(() => {
    ...
    for (const cat of CATEGORY_KEYS) {
      result[cat] = ALL_FEATURES.filter((f) => f.category === cat).map((f) => ({
        key: f.key,
        label: f.label,
        enabled: selectedConfig.features[f.key as keyof typeof selectedConfig.features] ?? false,
      }));
    }
    return result;
  }, [selectedConfig]);
  ...
  return (
    <div className={cn("mx-auto w-full", isModal ? "max-w-4xl" : "max-w-5xl")}>
      <div className="relative overflow-hidden rounded-3xl border border-vytal-border bg-vytal-card backdrop-blur-xl">
        ...
        <div className={cn("p-6", isModal ? "sm:p-8" : "sm:p-10")}>
          {/* Header */}
          <div className="mb-8 text-center">
            <h1 className={cn("font-bold tracking-tight text-vytal-green", isModal ? "text-2xl sm:text-3xl" : "text-3xl sm:text-4xl")}>
              {isModal ? t("ui.createNewOrg") : "VYTAL"}
            </h1>
            {!isModal && (
              <p className="mt-2 text-sm text-vytal-muted">{t("onboarding.setupSteps")}</p>
            )}
          </div>
              ...
              {STEP_LABELS.map((labelKey, i) => {
                ...
                return (
                  <div key={labelKey} className="flex flex-col items-center gap-1.5">
                    ...
                    <span className={cn("text-[11px] font-medium transition-colors duration-300", isActive ? "text-vytal-green" : "text-vytal-muted")}>
                      {t(labelKey)}
                    </span>
                  </div>
                );
          ...
          <div key={stepKey.current} className="onboarding-step-enter">
            ...
            {step === 1 && (
              <div>
                <h2 className="mb-1 text-center text-xl font-bold text-vytal-text sm:text-2xl">{t("onboarding.step1Title")}</h2>
                <p className="mx-auto mb-8 max-w-lg text-center text-sm text-vytal-muted">{t("onboarding.step1Subtitle")}</p>
                ...
                <div className="relative mx-auto mb-8 max-w-md">
                  ...
                  />
                  {searchQuery && (
                    <button type="button" onClick={() => setSearchQuery("")} className="absolute right-3 top-1/2 -translate-y-1/2 text-vytal-muted transition-colors hover:text-vytal-text">
                      <X className="h-4 w-4" />
                    </button>
                  )}
                </div>

                <div className={cn("grid gap-3", isModal ? "grid-cols-2 sm:grid-cols-3 md:grid-cols-4" : "grid-cols-2 sm:grid-cols-3 md:grid-cols-4")}>
                  {filteredTypes.map((config, i) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <span className={cn("text-[40px] leading-none transition-transform duration-300", isSelected && "scale-110")}>{emoji}</span>
                        <span className={cn("text-xs font-bold tracking-wide transition-colors duration-200", isSelected ? "text-vytal-green" : "text-vytal-text")}>
                          {t(`vertical.${config.type}`)}
                        </span>
                        <span className="line-clamp-1 text-[10px] leading-tight text-vytal-muted">{t(`vertical.${config.type}.desc`)}</span>
                      </button>
                    );
                  })}
                </div>
            ...
            {step === 2 && (
              <div>
                <h2 className="mb-1 text-center text-xl font-bold text-vytal-text sm:text-2xl">{t("onboarding.step2Title")}</h2>
                <p className="mx-auto mb-8 max-w-lg text-center text-sm text-vytal-muted">
                  {t("onboarding.step2Subtitle")} {selectedConfig?.terminology.organization.toLowerCase() ?? "space"}
                </p>

                <div className="grid grid-cols-1 gap-8 lg:grid-cols-5">
                  <div className="space-y-4 lg:col-span-3">
                    <FloatingInput label={t("onboarding.fieldName")} value={details.name} onChange={(v) => updateDetail("name", v)} required />
                    <div className="relative">
                      <div className="flex items-stretch">
                        <span className="flex items-center rounded-l-xl border border-r-0 border-vytal-border bg-vytal-bg3 px-3 text-xs font-medium text-vytal-muted">vytal.fit/</span>
                        <input
                          type="text"
                          value={details.slug}
                          onChange={(e) => { setSlugEdited(true); setDetails((prev) => ({ ...prev, slug: slugify(e.target.value) })); }}
                    ...
                    <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                      <FloatingInput label={t("onboarding.fieldEmail")} value={details.email} onChange={(v) => updateDetail("email", v)} type="email" required />
                      <FloatingInput label={t("onboarding.fieldPhone")} value={details.phone} onChange={(v) => updateDetail("phone", v)} type="tel" />
                    </div>
                    <FloatingInput label={t("onboarding.fieldAddress")} value={details.address} onChange={(v) => updateDetail("address", v)} />
                    <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                      <FloatingInput label={t("onboarding.fieldCity")} value={details.city} onChange={(v) => updateDetail("city", v)} />
                      <FloatingInput label={t("onboarding.fieldZipCode")} value={details.zipCode} onChange={(v) => updateDetail("zipCode", v)} />
                    </div>
                    <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                      <div>
                        <label className="mb-1.5 block text-[10px] font-semibold uppercase tracking-wider text-vytal-muted">{t("onboarding.fieldCountry")}</label>
                        <select value={details.country} onChange={(e) => updateDetail("country", e.target.value)} className="w-full appearance-none rounded-xl border border-vytal-border bg-vytal-bg2 px-4 py-3.5 text-sm text-vytal-text outline-none transition-all duration-200 focus:border-vytal-green/50 focus:ring-2 focus:ring-vytal-green/20">
                          {COUNTRIES.map((c) => (<option key={c.code} value={c.code}>{c.flag} {c.label}</option>))}
                        </select>
                      </div>
                      <div>
                        <label className="mb-1.5 block text-[10px] font-semibold uppercase tracking-wider text-vytal-muted">{t("onboarding.fieldCurrency")}</label>
                        <select value={details.currency} onChange={(e) => updateDetail("currency", e.target.value)} className="w-full appearance-none rounded-xl border border-vytal-border bg-vytal-bg2 px-4 py-3.5 text-sm text-vytal-text outline-none transition-all duration-200 focus:border-vytal-green/50 focus:ring-2 focus:ring-vytal-green/20">
                          {CURRENCIES.map((c) => (<option key={c.code} value={c.code}>{c.label}</option>))}
                        </select>
                      </div>
                    </div>
                    <div>
                      <label className="mb-1.5 block text-[10px] font-semibold uppercase tracking-wider text-vytal-muted">{t("onboarding.fieldTimezone")}</label>
                      <div className="flex items-center gap-2 rounded-xl border border-vytal-border bg-vytal-bg2 px-4 py-3.5">
                        ...
                        <span className="text-sm text-vytal-text">{details.timezone}</span>
                        <span className="ml-auto text-[10px] text-vytal-muted">auto</span>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="lg:col-span-2">
                    <div className="sticky top-6 rounded-2xl border border-vytal-border bg-vytal-bg2 p-6">
                      <p className="mb-5 text-center text-[10px] font-semibold uppercase tracking-widest text-vytal-muted">{t("onboarding.livePreviewLabel")}</p>
                      ...
                      <h3 className="mb-1 text-center text-lg font-bold text-vytal-text">{details.name || t("onboarding.noName")}</h3>
                      {selectedConfig && (
                      ...
                      <div className="space-y-2.5 border-t border-vytal-border pt-4">
                        {details.slug && (
                          <div className="flex items-center gap-2 text-xs text-vytal-muted"><Globe className="h-3.5 w-3.5 shrink-0" /><span className="truncate">vytal.fit/{details.slug}</span></div>
                        )}
                        {details.email && (
                          <div className="flex items-center gap-2 text-xs text-vytal-muted"><Mail className="h-3.5 w-3.5 shrink-0" /><span className="truncate">{details.email}</span></div>
                        )}
                        {details.phone && (
                          <div className="flex items-center gap-2 text-xs text-vytal-muted"><Phone className="h-3.5 w-3.5 shrink-0" /><span>{details.phone}</span></div>
                        )}
                        {(details.address || details.city) && (
                          <div className="flex items-center gap-2 text-xs text-vytal-muted"><MapPin className="h-3.5 w-3.5 shrink-0" /><span className="truncate">{[details.address, details.city, details.zipCode].filter(Boolean).join(", ")}</span></div>
                        )}
                      </div>
                    </div>
                  </div>
            ...
            {step === 3 && selectedConfig && (
              <div className="relative">
                <Confetti />
                <AnimatedCheckmark />
                <h2 className="mb-1 text-center text-2xl font-bold text-vytal-text sm:text-3xl">{t("onboarding.step3Title")}</h2>
                <p className="mx-auto mb-8 max-w-md text-center text-sm text-vytal-muted">{t("onboarding.step3Subtitle")}</p>
                  ...
                  <div className="flex items-start gap-4">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex flex-wrap items-center gap-2">
                        <h3 className="text-lg font-bold text-vytal-text truncate">{details.name || t("onboarding.noName")}</h3>
                        <span className="inline-flex shrink-0 items-center gap-1 rounded-full bg-vytal-green/10 px-2.5 py-0.5 text-[10px] font-semibold uppercase tracking-wider text-vytal-green">
                          {EMOJI_MAP[selectedConfig.type]} {t(`vertical.${selectedConfig.type}`)}
                        </span>
                      </div>
                      <p className="mt-0.5 text-sm text-vytal-muted truncate">{details.email}</p>
                      {details.slug && <p className="mt-0.5 text-xs text-vytal-muted">vytal.fit/{details.slug}</p>}
                    </div>
                  </div>
                </div>
                ...
                <div className="mx-auto mb-8 max-w-lg rounded-2xl border border-vytal-green/20 bg-vytal-green/[0.04] p-5">
                  <div className="mb-3 flex items-center gap-2">
                    ...
                    <span className="text-xs font-semibold uppercase tracking-wider text-vytal-green">{t("onboarding.terminologyPreview")}</span>
                  </div>
                  <p className="text-sm text-vytal-text">
                    {t("onboarding.terminologySentence")
                      .replace("{members}", selectedConfig.terminology.memberPlural.toLowerCase())
                      .replace("{sessions}", selectedConfig.terminology.sessionPlural.toLowerCase())
                      .replace("{instructors}", selectedConfig.terminology.instructorPlural.toLowerCase())
                      .split(/(\b(?:athletes|students|members|clients|swimmers|dancers|players|climbers|riders|runners|gymnasts|patients|lifters|participants|classes|sessions|trainings|lessons|rides|practices|runs|appointments|coaches|instructors|trainers|pts|teachers|therapists|sensei)\b)/i)
                      .map((part, idx) => {
                        ...
                        if (termLower.includes(lower)) {
                          return <span key={idx} className="font-semibold text-vytal-green">{part}</span>;
                        }
                        return part;
                      })}
                  </p>
                ...
                <div className="mx-auto mb-8 max-w-lg">
                  <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                    {CATEGORY_KEYS.map((cat) => {
                      ...
                      return (
                        <div key={cat} className="rounded-xl border border-vytal-border bg-vytal-bg2 p-4">
                          <h4 className="mb-3 text-[10px] font-semibold uppercase tracking-wider text-vytal-muted">{t(CATEGORY_I18N[cat])}</h4>
                          <div className="space-y-1.5">
                            {features.map((f) => (
                              <div key={f.key} className="flex items-center gap-2">
                                {f.enabled ? (
                                  <div className="flex h-4 w-4 items-center justify-center rounded-full bg-vytal-green/15"><Check className="h-2.5 w-2.5 text-vytal-green" /></div>
                                ) : (
                                  <div className="flex h-4 w-4 items-center justify-center rounded-full bg-vytal-bg3"><X className="h-2.5 w-2.5 text-vytal-muted/40" /></div>
                                )}
                                <span className={cn("text-xs", f.enabled ? "text-vytal-text" : "text-vytal-muted/50 line-through")}>{f.label}</span>
                              </div>
                            ))}
                          </div>
                        </div>
                ...
                <div className="mx-auto mb-2 max-w-lg text-center">
                  <div className="inline-flex items-center gap-2 rounded-full bg-vytal-bg2 border border-vytal-border px-4 py-2">
                    ...
                    <span className="text-xs text-vytal-muted">{t("onboarding.joinTeaser")}</span>
                  </div>
                </div>
              </div>
            )}
          ...
          <div className="mt-10 flex items-center justify-between">
            <button
              ...
              className={cn(
                ...
                step === 1 ? "cursor-not-allowed text-vytal-muted/30" : "text-vytal-muted hover:bg-vytal-bg3 hover:text-vytal-text"
              )}
            >
