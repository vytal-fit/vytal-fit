  ...
  return (
    <div className="space-y-6">
      ...
      <div className="rounded-2xl p-4 bg-vytal-bg2 border border-vytal-border">
        <div className="flex items-center justify-between mb-3">
          <div className="flex items-center gap-2">
            <TrendingUp size={14} className="text-vytal-green" />
          ...
          {weekDays.map((day) => {
            ...
            return (
              <div key={day.date} className="flex-1 flex flex-col items-center gap-1.5">
                <span className="text-[9px] font-medium uppercase text-vytal-muted">
                  {day.label}
                </span>
                <div
                  className={cn(
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {/* ── Next class card ── */}
        {nextClass ? (
          <div className="rounded-2xl overflow-hidden transition-all duration-200 hover:scale-[1.01] bg-vytal-bg2 border border-vytal-border">
            {/* Color accent bar */}
            <div
            ...
            <div className="p-4 space-y-3">
              <div className="flex items-center justify-between">
                ...
                <Link
                  href="/schedule"
                  className="flex items-center gap-1 text-[10px] font-semibold transition-opacity hover:opacity-70 text-vytal-green"
                >
              ...
              <div>
                <div className="flex items-center gap-2 mb-2">
                  ...
                  <span className="font-bold text-base text-vytal-text">
                ...
                <div className="grid grid-cols-2 gap-1.5 mb-3">
                  {[
                    ...
                    return (
                      <div key={i} className="flex items-center gap-1.5 text-[11px] text-vytal-muted">
                        <Icon size={11} />
          ...
          <div className="rounded-2xl p-6 flex flex-col items-center justify-center text-center gap-3 bg-vytal-bg2 border border-vytal-border">
            <Calendar size={28} className="text-vytal-muted" style={{ opacity: 0.4 }} />
            <p className="text-sm font-medium text-vytal-muted">
              {t("my.home.noClasses")}
            </p>
            <p className="text-xs text-vytal-muted" style={{ opacity: 0.6 }}>
              {t("my.home.restWell")}
            </p>
          </div>
        )}
        ...
        {todayWOD ? (
          <div className="rounded-2xl overflow-hidden transition-all duration-200 hover:scale-[1.01] bg-vytal-bg2 border border-vytal-border">
            ...
            <div className="p-4 space-y-3">
              ...
              <div>
                <h3 className="font-black text-xl leading-tight mb-1 text-vytal-green">
                  {todayWOD.title ?? "WOD do Dia"}
                </h3>
                {todayWOD.description && (
                  <p className="text-xs leading-relaxed mb-3 line-clamp-2 text-vytal-muted">
                    {todayWOD.description}
                  </p>
                )}
                ...
                {todayWOD.parts?.length > 0 && (
                  <div className="space-y-1.5">
                    {todayWOD.parts.slice(0, 3).map((part, i) => (
                      <div
                        key={i}
                        className="flex items-start gap-2 rounded-lg px-2.5 py-2 bg-vytal-bg3"
                      >
                        <div className="w-4 h-4 rounded flex items-center justify-center text-[9px] font-black shrink-0 mt-0.5 bg-vytal-green text-vytal-bg">
                          {i + 1}
                        </div>
                        <div className="min-w-0">
                          <p className="text-[11px] font-bold truncate text-vytal-text">
                            {part.name}
                          </p>
                          <p className="text-[10px] truncate text-vytal-muted">
                    ...
                    {todayWOD.parts.length > 3 && (
                      <p className="text-[10px] text-center text-vytal-muted">
          ...
          <div className="rounded-2xl p-6 flex flex-col items-center justify-center text-center gap-3 bg-vytal-bg2 border border-vytal-border">
            <Dumbbell size={28} className="text-vytal-muted" style={{ opacity: 0.4 }} />
            <p className="text-sm font-medium text-vytal-muted">
      ...
      >
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div className="w-9 h-9 rounded-xl flex items-center justify-center shrink-0 bg-[rgba(0,212,255,0.1)] border border-[rgba(0,212,255,0.2)]">
              <Users size={16} className="text-vytal-blue" strokeWidth={1.8} />
            </div>
            <div>
          ...
          <ArrowRight size={15} className="text-vytal-blue" />
        </div>
      </Link>
      ...
      >
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div className="w-9 h-9 rounded-xl flex items-center justify-center shrink-0 bg-[rgba(192,132,252,0.1)] border border-[rgba(192,132,252,0.2)]">
              <Dumbbell size={16} className="text-vytal-purple" strokeWidth={1.8} />
            </div>
            <div>
              <p className="text-sm font-bold text-vytal-text">
          ...
          <ArrowRight size={15} className="text-vytal-purple" />
        </div>
      </Link>
      ...
      >
        ...
        <div className="flex gap-2 flex-wrap">
          {[
            ...
            <div
              key={i}
              className="flex items-center gap-1.5 px-3 py-1.5 rounded-full transition-all duration-200 hover:scale-105 bg-[rgba(34,197,94,0.1)] border border-[rgba(34,197,94,0.2)]"
            >
              <span className="text-xs">{badge.icon}</span>
              <span className="text-[11px] font-semibold text-vytal-text">
                {badge.label}
              </span>
            </div>
          ))}
